...
export const renderRealTimeProducts = async (req, res) => {
    try {
        ...
        res.render("realtimeproducts", { productos: productosFormatted, isRegisterPage: true, user: req.session.user });
    } catch (error) {
...
export const renderProducts = async (req, res) => {
    try {
        ...
        const productosFormatted = productos.docs.map(producto => {
            const { _id, ...rest } = producto.toObject();
            return { _id, ...rest };
        });
        res.render("products", {
